...
const Dashboard: React.FC = () => {
  //criar os estados para poder controlar os 'inputs':
  const [minDate, setMinDate] = useState('');
  const [maxDate, setMaxDate] = useState('');
  const dispatch = useDispatch();

  //função para utilizar os 'buttons':
  const handleSetDates = (days: number) => {
    //'new Date' pega a data atual e subtrai 'days' para pegar as datas para trás
    setMinDate(format(addDays(new Date(), - days), 'yyyy-MM-dd'));
    setMaxDate(format(new Date(), 'yyyy-MM-dd'));
  }
  
  useEffect(() => {
    //a qlqr mudança do 'min ou max' vai rodar o 'dispatch'
