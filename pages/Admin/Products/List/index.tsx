...
const List: React.FC = () => {
  ...
  // se o search ou a página mudar vai alterar a url
  useEffect(() => {
    setUrl(
      defaultUrl +
  ...
  return (
    <AdminComponent>
      ...
      {
        data && data.products && data.products.length > 0 ? ( // se não chama o NoData
          <AdminListTable 
            first_title="Nome do produto" 
            second_title="Categorias" 
            third_title="Código" 
            fourth_title="Status"  
            meta={data.meta} // paginação
          >
            {
              data.products.map(product => (
                // para cada produto é add uma linha
                <tr key={product.id}>
                  <td>{product.name}</td>
                  <td>
                    { 
